    ...
    async def _render_internal(self, slide_data: dict, project_config: dict, output_dir: str = None) -> str:
        ...
        if image_path.startswith("http://") or image_path.startswith("https://"):
            try:
                ...
                temp_image_path = await asyncio.to_thread(self._save_temp_image, content)
                image_path = temp_image_path # Override for worker
                    
            except Exception as e:
                print(f"ERROR: [render_slide] Failed to download image {image_path}: {e}")
        ...
        try:
            ...
            if temp_image_path and os.path.exists(temp_image_path):
                try:
                    ...
                    print(f"DEBUG: [render_slide] Cleaned up temp image: {temp_image_path}")
                except Exception as e:
