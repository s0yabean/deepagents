...
def merge_todos(existing: Any, new_updates: Any) -> List[Dict[str, Any]]:
    ...
    if not isinstance(new_updates, list):
        raise ValueError(
            f"Invalid todo update: Expected a list, got {type(new_updates).__name__}."
        )
    ...
    merged = []
    existing_lookup = {
        (item.get("content"), item.get("owner")): item
    ...
    for new_item in new_updates:
        if not isinstance(new_item, dict) or "content" not in new_item:
            raise ValueError(
                "Invalid todo item: Each item must be a dictionary with a 'content' key."
            )
...
class AgentState(TypedDict):
    ...
    # Creative Brief (Central source of truth - avoid passing full JSON in tasks)
